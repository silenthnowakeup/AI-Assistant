...
async def handle_message(message: Message, state: FSMContext):
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "Ты помощник, который определяет ключевые личные ценности в тексте и сохраняет их. Учти, что ключевых ценностей в тексте может быть несколько, их также определи"},
                {"role": "user",
                 "content": f"Определи все ключевые ценности в следующем тексте и сохрани их: {input_text}"}
            ],
            functions=[
                {
                    "name": "save_value",
